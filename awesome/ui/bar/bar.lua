...
awful.screen.connect_for_each_screen(function(s)
	...
	apps_volume = function()
		utils.run_or_raise({ class = "Pavucontrol" }, true, "pavucontrol")
	end
	...
	local logoff_command = function()
		awesome.quit()
	end

	local shutdown = create_power_box(
		colorize_icon(beautiful.shutdown, beautiful.icon_normal),
		"Shutdown",
		100,
		40,
		dpi(10),
	...
	local restart = create_power_box(
		colorize_icon(beautiful.restart, beautiful.icon_normal),
		"Restart",
		100,
		40,
		dpi(10),
	...
	local logoff = create_power_box(
		colorize_icon(beautiful.logout, beautiful.icon_normal),
		"Logoff",
		100,
		40,
		dpi(10),
	...
	local awesome_icon = wibox.widget({
		{
			...
			{
				utils.vertical_pad(10),
				{
					markup = utils.colorize_text("ChocolateBread", beautiful.fg_contrast),
					font = "SF Pro Display Medium 13",
					widget = wibox.widget.textbox,
				},
				{
					markup = utils.colorize_text("User", beautiful.fg_contrast .. "70"),
	...
	local dark_toggle_wide = wibox.widget({
		...
		utils.horizontal_pad(12),
		{
			markup = utils.colorize_text("Dark Mode", beautiful.fg_normal),
	...
	s.mywidebox.widget = wibox.widget({
		...
		expand = "none",
		{ -- top
			utils.vertical_pad(dpi(15)),
			wrap_widget({
				logo_wide,
				left = dpi(15),
				widget = wibox.container.margin,
			}),
			utils.vertical_pad(dpi(150)),
			wrap_widget(s.mytaglist),
			utils.vertical_pad(dpi(120)),
			{
				layout = wibox.layout.align.horizontal,
				expand = "none",
				nil,
	...
	s.mywidebox:setup({
		...
		bg = beautiful.bg,
		shape = utils.rrect(beautiful.bar_radius),
		widget = wibox.container.background,
	})
	...
	s.mywibox:setup({
		...
		bg = beautiful.bg,
		shape = utils.rrect(beautiful.bar_radius - dpi(2)),
		widget = wibox.container.background,
	})
